...
mclock::time_point sta::util::Now()
{
    return mclock::now();
}

int64_t sta::util::ElapsedMillisFrom(const mclock::time_point& from)
{
    return ElapsedMillis(from, Now());
}

int64_t sta::util::ToMillis(const mclock::duration& duration)
{
...
mclock::duration sta::util::ToDuration(int64_t millis)
{
    return std::chrono::milliseconds(millis);
}

int64_t sta::util::ElapsedMillis(const mclock::time_point& from, const mclock::time_point& to)
{
    return ToMillis(to - from);
}

std::string sta::util::GetTimestampNow()
{
...
std::string sta::util::SimpleDurationFormat(const mclock::duration& duration, SimpleTimeFormatFlags flags)
{
    return SimpleMillisFormat(ToMillis(duration), flags);
}

std::string sta::util::SimpleMillisFormat(int64_t millis, SimpleTimeFormatFlags flags)
{
